...
public interface MessageRepository {
    ...
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    @Insert("INSERT INTO message_table(role,dialog_id,sender_id,sender,message,date,attachments,files_guid,file_name) " +
            "VALUES(#{message.role},#{message.dialogId},#{message.senderId},#{message.sender},#{message.message},#{message.date},#{message.attachments},#{message.filesGuid},#{message.fileName})")
    long saveAttachments(@Param("message") Message message);
}
